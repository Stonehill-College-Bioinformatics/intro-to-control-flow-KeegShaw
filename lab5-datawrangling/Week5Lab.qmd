...
```{r}
  delayed_flightsdf <- flights %>% 
  
  filter(arr_delay > 12 * 60) %>%
  count(origin,sort = TRUE)

glimpse(delayed_flightsdf)
...
```{r}
av_airt_JFK_LAX_df <- flights %>%
  
filter(origin == "JFK", dest == "LAX") %>%
  summarise(Avg_AirT = mean(air_time, na.rm = TRUE) / 60, N_flights = n())

glimpse(av_airt_JFK_LAX_df)
```
